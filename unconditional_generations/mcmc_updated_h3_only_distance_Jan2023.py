...
#distance_ratio = 50  #update back into 50 #update it into 55 only for heavey chain in index 0,2,4,5,6,8 #50 #40 forget to change last time # this is for eucudian distance btw encoded map of c-beta and c-beta
...
distance_ratio = 6 * (h3_end - h3_start)
...
def demo_func(fix_noise):# input batch size 1 
    ...
    dis = cal_h3_distance(target_test,samples)
    print(dis)
    return dis
    #return rmsd
...
#comb_num = len(target_coords)
   ...
   # saved_y = 99999
    ...
    for i in range(loops):
        ...
        if len(to_save_lst) > 0: 
            generated_samples = torch.cat(to_save_lst,0)
            pickle_out_W = open(path_g1+str(kkk)+"_"+name_f+"_to_save_lst_lst_dis50_save_memory_with_achor_ptrs_used_mcmc_on_heavychain_chain_h3_only_Jan08_2023.pickle","wb")
