...
#"_to_save_lst_lst_dis50_save_memory_with_achor_ptrs_used_mcmc_on_heavychain_chain_only_nov_dec1_index_all.pickle"
...
# this function take np_sample (generated data). replace the H3 region with the target H3 to do assembling.
...
dis_lst = []
def cal_h3_distance(one_test,samples):
#    target_sample = numpy.array(test_d[kkk]["data"])
    start = h3_pos_lst[kkk][0]
    end = h3_pos_lst[kkk][1]
    ...
    a_encode_c = one_test["data"][:,start:end,:]
    a_encode_r = one_test["data"][:,:,start:end]
    for i in range(len(samples)):
        #a_encode = one_test["data"][:1]
        b_encode_c = samples[i][:,start:end,:]
        b_encode_r = samples[i][:,:,start:end]
        d1 = np.linalg.norm(a_encode_c-b_encode_c)
        d2 = np.linalg.norm(a_encode_r-b_encode_r)
        dis_lst.append(d1+d2)
        distance = d1 + d2
        if distance < min_dis:
            min_dis = distance
...
h3_region = [96-20, 116+10]

def cal_h3_distance_f(one_test,samples):
#    target_sample = numpy.array(test_d[kkk]["data"])
    start = h3_pos_lst[kkk][0]
    end = h3_pos_lst[kkk][1]
    length = end - start
    ...
    for i in range(len(samples)):
        ...
        batch_mini_dis = 999
        k = 0
        for j in range(h3_region[0],h3_region[1]-length):
            b_encode_c = samples[i][:,j:j+length,:]
            b_encode_r = samples[i][:,:,j:j+length]
            d1 = np.linalg.norm(a_encode_c-b_encode_c)
            d2 = np.linalg.norm(a_encode_r-b_encode_r)
            #dis_lst.append(d1+d2)
            distance = d1 + d2
            if distance < batch_mini_dis:
                batch_mini_dis = distance
                k = j
        dis_lst.append((batch_mini_dis,k))
        if distance < min_dis:
            min_dis = distance
...
def cal_h3_distance_beta(one_test,samples):
#    target_sample = numpy.array(test_d[kkk]["data"])
    start = h3_pos_lst[kkk][0]
    end = h3_pos_lst[kkk][1]
    ch = 0
    min_dis =9999
    test_name = one_test["name"]
    a_encode_c = one_test["data"][ch,start:end,:]
    a_encode_r = one_test["data"][ch,:,start:end]
    for i in range(len(samples)):
        #a_encode = one_test["data"][:1]
        b_encode_c = samples[i][ch,start:end,:]
        b_encode_r = samples[i][ch,:,start:end]
        d1 = np.linalg.norm(a_encode_c-b_encode_c)
        d2 = np.linalg.norm(a_encode_r-b_encode_r)
        dis_lst.append(d1+d2)
        distance = d1 + d2
...
#    end = h3_pos_lst[kkk][1]
    ...
    min_dis =9999
    test_name = one_test["name"]
    a_encode = one_test["data"][0,start:end,:]
    for i in range(len(samples)):
        #a_encode = one_test["data"][:1]
        b_encode = samples[i][0,start:end,:]
...
#cal_h3_distance_beta(test_d[kkk],samples)
cal_h3_distance_f(test_d[kkk],samples)


print(min(dis_lst),(dis_lst.index(min(dis_lst))))#,(sum(dis_lst)/len(dis_lst)))
#print(dis_lst[784])
def generate():
    ...
    # the rest is new updated part on Jan 4th 2023
        ...
        print(seq)
        # the next three line for hacked.
