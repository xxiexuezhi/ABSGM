...
def run_minimization(npz,seq,n_iter,scriptdir,outPath,pose=None,angle_std=10,dist_std=0.5):
    ...
    if True:
        ...
        indices_to_design = [str(i+1) for i,c in enumerate(seq) if c == "A"]
    ...
    # For some reason FastDesign fails in the middle
    # while not outPath.joinpath("structure_after_design.pdb").is_file():
    #     try:
    #         rel_design.apply(pose0)
    #     except Exception:
    #         print("FastDesign failed... retrying")
    ...
    relax_round1.set_task_factory(tf)  # previously, turn off. let;s try to turn on
    ...
    relax_round2.set_task_factory(tf)  # same. previously, turn off. let's turn on
