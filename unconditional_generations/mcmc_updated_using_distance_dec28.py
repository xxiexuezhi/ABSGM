...
distance_ratio = 55  # update on Jan31.2023 change it into 55 for more generations #update back into 50 #update it into 55 only for heavey chain in index 0,2,4,5,6,8 #50 #40 forget to change last time # this is for eucudian distance btw encoded map of c-beta and c-beta
...
with open("4O5I_and_4XMP_mcmc.pkl","rb") as f:
...
#update name_f due to I change the name. Jan 31 2023 

name_f = test_name.split(".")[0].split(" ")[0]
...
#comb_num = len(target_coords)
   ...
   # saved_y = 99999
    ...
    for i in range(loops):
        ...
        if len(to_save_lst) > 1:
            generated_samples = torch.cat(to_save_lst,0)
            pickle_out_W = open(path_g1+str(kkk)+"_"+name_f+"_to_save_lst_lst_dis50_save_memory_with_ahcnor_ptrs_for_4O5Iand4XMP.pickle","wb")
