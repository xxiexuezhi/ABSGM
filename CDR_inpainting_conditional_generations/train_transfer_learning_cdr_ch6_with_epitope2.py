...
import time
from utils import random_mask_batch, get_condition_from_batch, selected_mask_batch,selected_mask_batch2,selected_mask_batch3
import shutil

import pickle

with open("protein_dataset_dataset3_l_with_epitope_match_cdr_4k_all_matching_info_add_after_masking_and_epitope_info_no_padding_midptr_64_Mar10_2023.pkl","rb") as f:
...
def main():
    ...
    parser.add_argument('--resume', type=str, default="transfer_learning_protein_dataset_dataset3_l_with_no_epitope_match_cdr_10k_all_matching_info_update_into_all_data_for_top_left_positions_with_no_padding_Mar10th")
   ...
   # dataset = ProteinDataset(config.data.dataset_path, config.data.min_res_num,
    ...
    #import pickle
    with open("transfer_learning_validation_set_protein_dataset_dataset3_l_with_epitope_match_cdr_4k_all_matching_info_add_after_masking_and_epitope_info_no_padding_midptr_64_Mar10_2023.pkl","wb") as fout:
        pickle.dump(test_ds,fout)
    ...
    for step in range(initial_step, config.training.n_iters + 1):
        ...
        mask_info = batch["ss_indices"]
        #print(mask_info)
        # Execute one training step
        batch = selected_mask_batch3(batch, mask_info, config)
        ...
        if step % config.training.eval_freq == 0:
            ...
            mask_info2 = eval_batch["ss_indices"]
            eval_batch = selected_mask_batch3(eval_batch, mask_info2, config)
        ...
        if step != 0 and step % config.training.snapshot_freq == 0 or step == config.training.n_iters:
            ...
            if config.training.snapshot_sampling:
                ...
                condition = get_condition_from_batch(config, eval_batch, eval_batch["ss_indices"]) #adding mask info here as the 3rd para. feb 11 2023
