...
import time
from utils import random_mask_batch, get_condition_from_batch, selected_mask_batch,selected_mask_batch2
import shutil

import pickle

with open("heavychain_ch8_with_specify_h1_h2_h3.pkl","rb") as f:
...
def main():
   ...
   # dataset = ProteinDataset(config.data.dataset_path, config.data.min_res_num,
    ...
    for step in range(initial_step, config.training.n_iters + 1):
        ...
        batch = selected_mask_batch2(batch, mask_info, config)
        ...
        if step % config.training.eval_freq == 0:
            ...
            eval_batch = selected_mask_batch2(eval_batch, mask_info2, config)
